...
public class SpringAIAgent extends LocalAgent {
    ...
    // ★ Text-based [TOOL_CALL] parsing: accumulate chunks and convert to structured tool calls
    private final java.util.concurrent.atomic.AtomicBoolean inTextToolCall = new java.util.concurrent.atomic.AtomicBoolean(false);
    private final StringBuilder textToolCallBuffer = new StringBuilder();
    ...
    /**
     * 技能注册表（可选）。用于 httpRequest URL 校验。
     * 如果不设置，跳过后端 URL 校验（仅依赖前端校验）。
     */
    ...
    private void onEvent(
        ...
        if (StringUtils.hasText(evt.getResult().getOutput().getText())) {
            ...
            // ★ Check for [TOOL_CALL] blocks BEFORE the tag filter strips them
            if (text.contains("[TOOL_CALL]") || text.contains("[tool_call]")) {
                inTextToolCall.set(true);
            }
            if (inTextToolCall.get()) {
                synchronized (textToolCallBuffer) {
                    textToolCallBuffer.append(text);
                    String buffered = textToolCallBuffer.toString();
                    int endIdx = buffered.indexOf("[/TOOL_CALL]");
                    if (endIdx < 0) endIdx = buffered.indexOf("[/tool_call]");
                    if (endIdx >= 0) {
                        inTextToolCall.set(false);
                        var parsedCalls = parseTextToolCalls(buffered);
                        if (!parsedCalls.isEmpty()) {
                            log.info("[SpringAIAgent] ★ Parsed {} text tool calls from stream", parsedCalls.size());
                            for (var tc : parsedCalls) {
                                String tcId = "text_tc_" + UUID.randomUUID().toString().substring(0, 8);
                                this.emitEvent(toolCallStartEvent(messageId, tc.name(), tcId), subscriber);
                                this.emitEvent(toolCallArgsEvent(tc.arguments(), tcId), subscriber);
                                this.emitEvent(toolCallEndEvent(tcId), subscriber);
                                deferredEvents.add(toolCallStartEvent(messageId, tc.name(), tcId));
                                deferredEvents.add(toolCallArgsEvent(tc.arguments(), tcId));
                            }
                        }
                        textToolCallBuffer.setLength(0);
                    }
                }
                return; // Don't emit [TOOL_CALL] text to frontend
            }
     ...
     */
    private List<AssistantMessage.ToolCall> parseTextToolCalls(String text) {
        List<AssistantMessage.ToolCall> result = new ArrayList<>();
        if (text == null || text.isEmpty()) return result;
        String lower = text.toLowerCase();
        int start = 0;
        while (true) {
            int blockStart = lower.indexOf("[tool_call]", start);
            if (blockStart < 0) break;
            int blockEnd = lower.indexOf("[/tool_call]", blockStart);
            if (blockEnd < 0) break;
            String block = text.substring(blockStart + 11, blockEnd).trim();
            start = blockEnd + 12;
            String toolName = null;
            for (String pat : new String[]{"tool => \"", "name => \""}) {
                int ns = block.indexOf(pat);
                if (ns >= 0) {
                    ns += pat.length();
                    int ne = block.indexOf("\"", ns);
                    if (ne > ns) { toolName = block.substring(ns, ne); break; }
                }
            }
            if (toolName == null) continue;
            String args = "{}";
            for (String pat : new String[]{"args => ", "args: "}) {
                int as = block.indexOf(pat);
                if (as >= 0) {
                    as += pat.length();
                    int bs = block.indexOf("{", as);
                    if (bs >= 0) {
                        int depth = 0; int ae = -1;
                        for (int i = bs; i < block.length(); i++) {
                            if (block.charAt(i) == '{') depth++;
                            else if (block.charAt(i) == '}') depth--;
                            if (depth == 0) { ae = i + 1; break; }
                        }
                        if (ae > bs) args = cleanTextArgs(block.substring(bs, ae));
                    }
                    break;
                }
            }
            String callId = "text_tc_" + UUID.randomUUID().toString().substring(0, 8);
            result.add(new AssistantMessage.ToolCall(callId, "function", toolName, args));
            log.info("[SpringAIAgent] Parsed text tool call: {} args={}", toolName, args);
        }
        return result;
    }

    private String cleanTextArgs(String raw) {
        raw = raw.trim();
        if (raw.startsWith("{") && raw.endsWith("}")) {
            try { new com.fasterxml.jackson.databind.ObjectMapper().readTree(raw); return raw; }
            catch (Exception e) { /* not valid JSON, fall through */ }
        }
            ...
            if (raw.charAt(i) == '-' && i + 1 < raw.length() && raw.charAt(i + 1) == '-') {
                ...
                if (i < raw.length()) {
                    ...
                    if (c == '"' || c == '\'') {
                        i++;
                        int vs = i;
                        while (i < raw.length() && raw.charAt(i) != c) i++;
        ...
        try { return new com.fasterxml.jackson.databind.ObjectMapper().writeValueAsString(params); }
    ...
    private boolean executeToolCallsAndReRun(RunAgentInput input, AgentSubscriber subscriber, String messageId, List<BaseEvent> deferredEvents, int toolExecutionCount) {
        ...
        for (AssistantMessage.ToolCall toolCall : toolCalls) {
            ...
            if (toolCallback != null) {
                ...
                // 对 httpRequest 进行后端 URL 校验：如果 URL 不合法，直接返回错误，
                // 让 LLM 看到错误后自行纠正（调用 loadSkill 获取正确 URL）。
                if ("httpRequest".equals(toolName) && this.skillRegistry != null) {
                    String urlError = validateHttpRequestUrl(toolArguments);
                    if (urlError != null) {
                        log.warn("[SpringAIAgent] httpRequest URL 校验失败: {} — {}", toolArguments, urlError);
                        String errorResult = "[URL_VALIDATION_ERROR] " + urlError;
                        this.emitEvent(toolCallResultEvent(toolCall.id(), errorResult, messageId, Role.tool), subscriber);
                        toolResponses.add(new ToolResponseMessage.ToolResponse(toolCall.id(), toolName, errorResult));
                        continue;
                    }
                }
    ...
    /**
     * 校验 httpRequest 工具参数中的 URL 是否合法。
     * @return null 如果合法，否则返回错误消息
     */
    private String validateHttpRequestUrl(String toolArguments) {
        if (toolArguments == null || toolArguments.isEmpty()) return null;
        try {
            var mapper = new com.fasterxml.jackson.databind.ObjectMapper();
            var node = mapper.readTree(toolArguments);
    ...
    public static class Builder {
         ...
         */
        /**
         * 设置技能注册表，用于 httpRequest URL 校验。
         *
         * @param skillRegistry the SkillRegistry instance
         * @return this builder instance for method chaining
         */
        public Builder skillRegistry(final SkillRegistry skillRegistry) {
