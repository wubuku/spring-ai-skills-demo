# Token 透传测试报告（历史记录，2026-03-16）

> **文档状态**: 带日期的历史测试报告，不是当前架构或当前测试结果的唯一来源。
> 当前认证是 Demo Base64 token，不是生产 JWT；当前前端端口为 `4000`，AG-UI 工具边界和验证命令请以 [文档总索引](docs/README.md) 为准。
> 本文中的“JWT”是历史命名，保留它是为了与当时的测试脚本和日志对应。
...
### 架构决策：用户态 Token 透传的能力边界

#### 核心原则

**把「代表用户访问后端 API」的能力，限定到你能完全控制线程模型的场景**。

#### 能力矩阵

| 场景 | 用户态 Token 透传 | 说明 |
|------|-----------------|------|
| **同步 Chat**（不走 Reactor，直接在 Servlet/虚拟线程上执行） | ✅ 支持 | 整个请求在同一线程上，ThreadLocal 可控 |
| **自定义 Executor/线程池**（自己写的、可控的） | ✅ 支持 | set + clear 可以严格做到 |
| **AG-UI + SSE + Spring AI** | ❌ 不支持 | Reactor/boundedElastic 线程池由框架控制，无法可靠掌控上下文 |

#### 不支持的原因

1. **Reactor Hook 不可靠**：Spring AI 使用 `Schedulers.onScheduleHook` 捕获 boundedElastic 线程上下文，但：
   - 首次调用可能捕获失败（竞态）
   - 清理时机难以精确控制

2. **线程池由框架控制**：boundedElastic 是 Spring Reactor 的内部线程池，开发者无法：
   - 精确控制任务提交时机
   - 保证每次都正确设置/清理 Context

3. **安全风险**：不清理会泄漏用户 Token，清理会导致首次调用失败

#### 推荐方案

**AG-UI + SSE 场景：使用服务级 Token（Service Token）**

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   前端用户      │     │  Spring AI     │     │   后端 API      │
│  (匿名/用户A)  │────▶│  (Agent/Tools) │────▶│  (服务Token)    │
└─────────────────┘     └─────────────────┘     └─────────────────┘
```

- AI 助手使用**服务级 Token** 调用后端 API
- 不再尝试透传用户 JWT
- 后端 API 根据服务 Token 授权，AI 的操作视为系统行为

> 本报告诚实记录此架构决策：AG-UI + SSE + Spring AI 场景下，放弃用户态 Token 透传，改用服务级 Token。
...
| **宠物 API 查询** | **✅ 已测试** | **100%** |
| **前端流式切换功能** (2026-04-11) | **✅ 已测试** | **100%** |

**总体通过率**: 19/19 测试通过 (100%)
